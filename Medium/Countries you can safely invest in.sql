...
**Query #1**
    ...
    select c.name  from 
    (Country c left join Person p on c.country_code = substring(p.phone_number, 1, 3)) 
    left join cte on cte.caller_id = p.id
    group by name
    having avg(cte.duration) > (select avg(duration) as global_average from Calls);
...
[View on DB Fiddle](https://www.db-fiddle.com/f/5EQuFRHq5HTQA923j2r9CQ/2)